...
const EventHostDashboard = () => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/events/${hostId}`
        );
  ...
  const myResellRequests = [
    {
      id: 4,
      eventName: "Techno Festival 2025",
      buyerName: "John Doe",
      requestedPrice: "$39",
  ...
  if (loading)
    return (
      <div className="text-center text-purple-400 mt-20 text-xl">
        Loading...
      </div>
    );
  if (error)
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center">
          <div className="flex-1 text-center">
            <h1 className="text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-purple-300 via-pink-500 to-indigo-400 animate-fade-in">
              My Events Dashboard
            </h1>
          </div>
          <button
            onClick={() => navigate("/create-event")}
            className="bg-gradient-to-r from-purple-600 to-pink-500 text-white px-6 py-3 rounded-full font-semibold text-base tracking-wide transition-all duration-300 hover:shadow-lg"
          >
            Create Event
          </button>
        </div>
          ...
          ? displayedEvents.map((event) => (
              ...
              >
                <div className="relative">
                  <img
                    src={event.images?.[0]?.url || "/img/fallback-image.jpg"} // Fixed: Changed posterImages to images
